...
from utils.messages import NO_ACHIEVEMENTS, NO_WORKOUT_HISTORY
    ...
    if current_day and current_status:
        status_controls.append(
            status_badge(
                ...
                "cyan" if current_status == "current" else ("blue" if current_status == "stopped" else "aqua"),
            )
        )
        ...
        ft.Column(
            ...
            controls=[
                ...
                ft.Row(
                    spacing=12,
                    controls=[
                        neon_button("Continue Today's Activity", ft.Icons.PLAY_CIRCLE_FILL_ROUNDED, lambda _: go(page, "/user/activity")),
                        soft_button("Plan Timeline", ft.Icons.TIMELINE, lambda _: go(page, "/user/timeline")),
                    ],
                ),
            ],
        )
